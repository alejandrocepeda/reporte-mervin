...
$.qubitAjax = function(url,options){
    ...
    if (settings.loadAnimation){

    	$('#load-qubit-ajax').remove();

    	$("<div id='load-qubit-ajax' class='loading_home hide navbar-fixed-top'><div class='load-message'>Cargando <i class='fa fa-circle-o-notch fa-spin'></i></div></div>").appendTo('body');
    	$("#load-qubit-ajax").fadeIn("slow");
    }
	...
	function _remove(){
		$("#load-qubit-ajax").fadeOut("slow", function() {
    		$(this).remove();
		});	
	}
